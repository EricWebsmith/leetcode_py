...
class Solution:
    def minNumberOfSemesters(self, n: int, relations: list[list[int]], k: int) -> int:
        ...
        while q:
            ...
            available_pres = [a for a in available if (a &
                              prerequisites_bitmask) > 0]
            available_not_pres = [a for a in available if (a &
                                  prerequisites_bitmask) == 0]
            if len(available_pres) <= k:
                ...
                taken |= bit_or(available_pres) | \
                    bit_or(available_not_pres[:k-len(available_pres)])
