...
class Solution:
    def categorizeBox(self, length: int, width: int, height: int, mass: int) -> str:
        ...
        bulky = length >= 10_000 or width >= 10_000 or height >= 10_000 or mass >= 10_000 \
...
def test(testObj: unittest.TestCase, length: int, width: int, height: int, mass: int, expected: str) -> None:
    so = Solution()
